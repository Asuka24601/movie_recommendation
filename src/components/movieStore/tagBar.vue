<template>
  ...
  <div class="monitor-list" v-show="MF.get_catergory().length > 0">
    <!-- 左边按钮 -->
    ...
    <div :id="String(listId) + 'list-box'" class="list-box">
      <div :id="String(listId) + 'list'" class="list" :style="'width:' + listWidth + ';'">
        ...
        <el-tag class="list-item"
                :id="String(listId) + 'mvCard'"
                v-for="item in MF.catergory"
                :key="item"
                type="primary"
                closable
                @close="MF.remove_catergory(item)"
                size="large">
          {{ item }}</el-tag>
...
export default {
  ...
  methods: {
    ...
    scrollLeft() {
      ...
      const allLength = itemLength * this.MF.catergory.length;
